...
function updateCameraPosition()
  ...
  if not pos or pos.x == 0 or pos.y == 0 then return end
  if not minimapWidget:isDragging() then
    local ok, err
    if not fullmapView then
      ok, err = pcall(function() minimapWidget:setCameraPosition(pos) end)
    end
    if ok ~= false then
      pcall(function() minimapWidget:setCrossPosition(pos) end)
    end
  end
end
