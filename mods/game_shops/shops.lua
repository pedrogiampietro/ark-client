...
-- Usando o mesmo backend do site, via HTTP para evitar loops de redirect HTTPS no cliente.
...
local function parseShopCard(data)
    ...
    -- Largura estimada: cipsoftFont ~8px/char, 2px/espaço
    local estimatedW = 0
    for i = 1, #msg do
        estimatedW = estimatedW + (msg:sub(i, i) == ' ' and 2 or 8)
    end
    card:setWidth(estimatedW + 10)
    label:setText(msg)
